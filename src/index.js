...
function makeResolvers(tables) {
  ...
  tables.forEach(t => {
    if(t !== undefined) {
      rtn.Query[t.toLowerCase()] = async (parent, args, ctx) => { return await getValues(parent, ctx, t, args) };
      rtn.Query[pluralize(t.toLowerCase(), 2)] = async (parent, args, ctx) => { return await getValues(parent, ctx, t, args) };
      ...
      for(let i = 0; i < fields.length; i++) {
        if(fields[i].endsWith("_id")) {
          ...
          const objRelation = relation.charAt(0).toUpperCase() + relation.slice(1);
          rtn[t][relation] = async (parent, args, ctx) => { return await getValues(parent, ctx, objRelation, args) };
        }
      }
    }
  });
